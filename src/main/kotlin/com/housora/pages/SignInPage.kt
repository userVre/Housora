        ...
        section("auth-page") {
            div("auth-container") {
                div("auth-left") {
                    ...
                    h1("auth-heading") { attributes["data-i18n"] = "auth.welcome"; +"WELCOME TO HOUSORA" }
        ...
        script {
            unsafe {
                ...
                (function() {
                    function bindAuthButtons() {
                        ...
                        if (googleBtn && googleBtn.dataset.authBound !== 'true') {
                            ...
                            googleBtn.addEventListener('click', function() {
                                if (window.HousoraAnalytics) window.HousoraAnalytics.track('login_started', { method: 'google' });
                                window.housoraOpenAuth('signin');
                            });
                        }
                        if (appleBtn && appleBtn.dataset.authBound !== 'true') {
                            appleBtn.dataset.authBound = 'true';
                            appleBtn.addEventListener('click', function() {
                                if (window.HousoraAnalytics) window.HousoraAnalytics.track('login_started', { method: 'apple' });
                                window.housoraOpenAuth('signin');
                            });
                        }
                        if (emailBtn && emailInput && emailBtn.dataset.authBound !== 'true') {
                            ...
                            emailBtn.addEventListener('click', function() {
                                ...
                                if (email && emailInput.checkValidity()) {
                                    ...
                                    if (window.HousoraAnalytics) window.HousoraAnalytics.track('login_started', { method: 'email' });
                                    window.housoraOpenAuth('signin');
                                } else {
                                    emailInput.focus();
                    ...
                    // If Clerk is already ready (Layout loaded first), bind immediately
                    bindAuthButtons();
                    if (window.Clerk && window.Clerk.user !== undefined) {
                        bindAuthButtons();
                    }

                    // Otherwise wait for the clerk:ready event
                    window.addEventListener('clerk:ready', function(e) {
                        var Clerk = e.detail.clerk;
                        // Preserve a safe in-site destination after sign-in.
                        if (Clerk.user) {
