...
const OUTPUT_DIR = path.join(__dirname, 'websitenewscreenshoots');
...
async function screenshotPage(browser, route, index, total) {
  ...
  try {
    ...
    console.log(`  [${index}/${total}] DESKTOP ${route} -> FAIL (${error.message.slice(0, 60)})`);
  }
  ...
  try {
    ...
    console.log(`  [${index}/${total}] MOBILE  ${route} -> FAIL (${error.message.slice(0, 60)})`);
  }
