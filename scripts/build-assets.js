...
function buildVendorAssets() {
  ...
  // Clerk UI is loaded from the configured instance Frontend API by
  // clerk-bootstrap.js. Do not vendor a second UI bundle: apart from being
  // obsolete, it can initialise a separate client tree alongside ClerkJS.
  // Remove stale output from older asset builds so static exports cannot keep
  // serving it after this pipeline completes.
  fs.rmSync(path.join(STATIC_DIR, 'vendor', 'clerk-ui'), { recursive: true, force: true });
...
async function buildImageAssets() {
  ...
  const responsiveFile = path.join(STATIC_DIR, 'assets', 'responsive-images.json');
  // Startup must be quick and offline-safe. The committed manifest is the
  // authoritative production derivative set; regeneration is an explicit
  // maintenance operation (ASSETS_REBUILD_IMAGES=1), not a side effect of
  // every local server start.
  if (process.env.ASSETS_REBUILD_IMAGES !== '1' && fs.existsSync(responsiveFile)) {
    const cachedManifest = JSON.parse(fs.readFileSync(responsiveFile, 'utf8'));
    const cachedImages = cachedManifest.images || {};
    for (const entry of Object.values(cachedImages)) {
      for (const format of ['avif', 'webp']) {
        for (const candidate of entry.sources?.[format] || []) {
          if (!fs.existsSync(sourceFileFor(candidate.path))) {
            throw new Error(`Missing responsive asset ${candidate.path}. Run ASSETS_REBUILD_IMAGES=1 bun run assets:build.`);
          }
        }
      }
    }
    const cachedPlaceholders = fs.existsSync(registryFile)
      ? (JSON.parse(fs.readFileSync(registryFile, 'utf8')).assets || []).length
      : 0;
    return { images: Object.keys(cachedImages).length, placeholders: cachedPlaceholders, cached: true };
  }
  const previous = fs.existsSync(registryFile) ? JSON.parse(fs.readFileSync(registryFile, 'utf8')).assets : [];
  const previousResponsive = fs.existsSync(responsiveFile)
    ? JSON.parse(fs.readFileSync(responsiveFile, 'utf8')).images || {}
    : {};
  ...
  for (const publicPath of imagePaths()) {
    ...
    // Responsive derivatives are deterministic. Reuse a complete cache when
    // every variant is newer than its source image; source edits and missing
    // variants naturally invalidate this fast path.
    const cached = previousResponsive[publicPath];
    const cachedSources = cached && cached.sources;
    const variantsAreCurrent = cached && cachedSources && ['avif', 'webp'].every(format =>
      Array.isArray(cachedSources[format]) && cachedSources[format].length > 0 &&
      cachedSources[format].every(candidate => {
        const candidateFile = sourceFileFor(candidate.path);
        return fs.existsSync(candidateFile) && fs.statSync(candidateFile).mtimeMs >= fs.statSync(file).mtimeMs;
      })
    );
    if (variantsAreCurrent) {
      if (placeholder) {
        placeholders.push({
          path: publicPath,
          width: cached.width,
...
async function main() {
  ...
  console.log('[assets] vendor assets: start');
  const vendor = buildVendorAssets();
  console.log(`[assets] vendor assets: complete in ${Date.now() - startedAt}ms`);
  const imageStartedAt = Date.now();
  console.log('[assets] responsive images: start');
  const images = await buildImageAssets();
  console.log(`[assets] responsive images: complete in ${Date.now() - imageStartedAt}ms`);
  const metadataStartedAt = Date.now();
  console.log('[assets] metadata: start');
  const metadata = await buildMetadataAssets();
  console.log(`[assets] metadata: complete in ${Date.now() - metadataStartedAt}ms`);
  console.log(`[assets] total: ${Date.now() - startedAt}ms`);
